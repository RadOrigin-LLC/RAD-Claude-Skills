...
Standalone skills packaged for [claude.ai](https://claude.ai). Import as a ZIP via **Settings > Customize > Skills**, add to a Project, or attach to any conversation. Two are Claude.ai counterparts of marketplace plugins (`rad-brainstormer`, `rad-seo-optimizer` → `rad-seo-aeo-reviewer`); one is Claude.ai-only after the matching plugin was retired (`rad-writer` — see the retirement note above).

| Skill | Source | ZIP | What It Does |
|-------|--------|-----|-------------|
| [rad-writer](skills/rad-writer/) | Claude.ai-only (plugin retired) | `rad-writer.zip` | Domain-aware writing and editorial review across 9 content types, AI pattern avoidance, voice profiling |
